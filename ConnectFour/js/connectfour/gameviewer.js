define(['jshelper'], function(jshelper){
        ...
        nGameContainerWidth = Math.round(oGameContainer.offsetWidth);
        nGameContainerHeight = Math.round(oGameContainer.offsetHeight);
        nGameContainerLeft = $(oGameContainer).offset().left;
        nGameContainerTop = $(oGameContainer).offset().top;
        nGameColumnWidth = Math.round(nGameContainerWidth / nGameDimensionX);
        nGameColumnVerticalGamePieceSpace = Math.round(nGameContainerHeight / nGameDimensionY);
        nHeaderHeight = oHeader.offsetHeight;
        nGamePieceDimension = Math.round(0.90 * nGameColumnVerticalGamePieceSpace);
    ...
    function GameViewer(oOptions){
        ...
        var _buildBoardHTML = function(){
            ...
            for (var i = 0; i < nGameDimensionX; i++) {
                ...
                oColumn.id = i;
                $(oColumn).css({
                    "width": Math.round(100/nGameDimensionX) + "%"
                });
		...
		var _renderHighlighterForDropPosition = function(){
			...
			for (var i = 0; i < nGameDimensionX; i++) {
				...
				nX0 = i*nGameColumnWidth + (nGameColumnWidth - nDiameter)/2;
				//For each row in that col, find the center.
				for (var j = 0; j < nGameDimensionY; j++) {
					nY0 = j*nGameColumnVerticalGamePieceSpace + (nGameColumnVerticalGamePieceSpace - nDiameter)/2;
                    ...
                    $(eDropHighlighterCircle).css({
                        "width": Math.round(nDiameter),
						"height": Math.round(nDiameter),
						"position": "absolute",
						"bottom": Math.round(nY0),
						"left": Math.round(nX0)
            ...
            $('#interactionColumnContainer').on('mousemove', function(event){
                if (self.getBooleanIsFloatingGamePiece()) {
                    ...
                    $((self.getCurrentGamePiece()).getElement()).css({
                        left: (event.pageX - _nGameContainerLeft + 6 - nGamePieceDimension / 2), /* add 6px for gameContainer margin */
                        top: "20px"
                    });
                }
            });//end mousemove
